# karan7.py
# This program is part of my submission for CSE 415
# Assignment 5 Game Playing Agents (Option B: Individual Work and K-in-a-Row)
# Karan Murthy
# Email: karan7@uw.edu
# StudentID: 1627346
# (C) 2017.
    ...
    try:
        ...
        if next_move == my_side:
            ...
            for state in generateSuccessors(current_state, next_move):
                ...
                alpha = max(alpha, threshold_score)
                if beta <= alpha:
                    break    
            ...
            for state in generateSuccessors(current_state, next_move):
                ...
                beta = min(beta, threshold_score)
                if beta <= alpha:
                    break
            return [next_state, beta]
...
def makeMove(currentState, currentRemark, timeLimit=5):
    ...
    depth_limit = math.ceil(k_to_win / 2)
...
def retrieveScore(my_count, opponent_count, num):
    count_difference = my_count - opponent_count
    if count_difference >= -math.ceil(k_to_win/2):
        return (count_difference) * pow(10, abs(count_difference))
    return (count_difference) * pow(20, num)
